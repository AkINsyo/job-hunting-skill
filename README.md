...
将 `seekmywork/` 目录复制到你的 Agent Skills 目录即可。SKILL.md 遵循 [Agent Skills 规范](https://agentskills.io/specification)，兼容任何支持该规范的 Agent。
...
```bash
# 查看帮助（推荐先看）
python seekmywork/scripts/fetch_jobs.py --help
...
# 批量拉取（配置文件见 assets/companies.json）
python seekmywork/scripts/fetch_jobs.py \
